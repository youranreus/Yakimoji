...
- [x] 建立 support 侧按 task ID 打开人工操作历史的独立入口 (AC: 1, 3)
  - [x] 新增独立 support route / loader，避免把 3.4 混入 creator workspace 或 6.2 审计总览
...
- [x] 渲染结构化 support history 视图 (AC: 2, 3)
  ...
  - [x] 保持与 3.3 support diagnostics 的视觉语言一致，但不混入 6.2 access audit / deliverable 审计语义
...
- 本 story 只交付 support 侧基于 task ID 的人工确认与人工介入历史，不扩展到 6.2 的最小审计记录总览。
...
- `tasks/:taskId/audit` 已承载 6.2 的最小审计记录与访问审计，不应直接作为 3.4 页面复用。
...
- 2026-06-20: Story artifact recreated because the previous implementation path mixed 3.4 scope with 6.2 audit scope.
...
- Added a support history card that shows task context, actor, request_id, and retention caveats without mixing in the 6.2 access-audit surface.
...
- Support history is now scoped to human actions only, so 3.4 no longer bleeds retry or 6.2 access-audit semantics into the page contract.
