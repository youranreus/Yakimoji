...
移动端的定位不是完整复刻桌面工作台，而是提供轻量跟进能力。它应支持查看任务进度、接收通知、执行必要确认，以及下载交付结果，以满足用户不在桌面前时的基本操作需求。
...
在移动端，导航应进一步压缩，但不改变逻辑。任务列表、任务详情、关键确认与恢复入口应保持可达，只是减少并列信息量，而不是改变整体产品心智。
...
移动端定位为轻量承接层，而不是完整生产端。移动端应支持查看任务进度、处理必要确认、下载交付结果，以及在异常场景下执行关键恢复动作。其目标不是容纳全部工作台复杂度，而是保证用户在离开桌面后仍能维持流程连续性和基本掌控感。
...
在这些断点之间，设计不应只做视觉缩放，而应基于信息优先级进行结构调整。桌面端允许更多并列信息与持续状态可见；平板端收敛为更清楚的分区块视图；移动端则优先保留状态、异常、确认与结果这四类关键对象，并减少同时展示的信息层级。
...
此外，移动端所有主要触控目标都应满足至少 44x44 的最小点击面积，以适应查看状态、确认动作、下载结果和异常恢复的操作场景。错误信息与恢复动作必须在语义和视觉上都保持明确，确保用户在任何设备上都能迅速理解问题并采取下一步行动。
