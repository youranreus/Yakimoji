...
UX-DR2: 移动端必须支持轻量创建任务、查看详情、处理低置信度确认与下载交付物。
...
UX-DR9: 实现陌生频道决策面板，并列提供复用已有预设与创建新预设两条路径。
...
### Story 1.4: Background Processing Lifecycle and Task Status Ledger

As a 创作者,
I want 在任务提交后看到统一的处理状态、阶段时间线和失败状态,
So that 我能知道任务当前进展并在出错时理解发生了什么。
...
**And** 同步方式可以通过 SSE、轮询兜底或两者结合实现，但不能要求用户手动刷新才能获得正常状态更新
...
**Given** 任务进入失败终态或处理中断
**When** 创作者查看任务列表或任务详情
**Then** 页面必须显示明确的失败状态与可读失败说明
**And** 系统必须为该失败保留可机器读取的原因代码与可追踪的 request_id 或等价诊断标识

**Given** 任务在后台有多次状态流转
**When** 创作者查看任务时间线
**Then** 时间线必须至少保留关键状态节点、发生时间和对应阶段说明
**And** 这些事件数据应可作为后续支持排障与运营审计的基础，而不是只存在前端临时状态中
...
### Story 1.5: Completed Deliverables and Secure Result Access
...
### Story 1.6: Creator Workspace Shell and Quality Baseline

As a 创作者,
I want 获得最小可用的桌面工作台壳层与质量基线,
So that 后续任务、预设、状态和交付功能都运行在一致、清晰且可访问的基础之上。

**Acceptance Criteria:**

**Given** 创作者已登录并进入桌面端工作台
**When** 首页首次加载受保护任务工作区
**Then** 工作台首屏必须满足已定义的可交互性能目标
**And** 页面结构必须优先呈现任务入口、任务列表或最近任务上下文等高价值内容

**Given** 系统尚未进入复杂业务功能实现
**When** 创作者首次进入工作台
**Then** 系统必须至少提供最小路由壳、全局导航、主内容区和空状态承载位
**And** 本 story 不负责实现完整的任务、预设、运营或支持业务流本身

**Given** 创作者在工作台执行高频动作
**When** 创作者浏览任务列表、筛选、进入详情或触发下载
**Then** 这些高频交互必须满足已定义的可感知响应目标
**And** 系统不得因粗暴全量刷新或过量数据加载破坏工作台流畅性

**Given** 工作台存在历史任务数据
**When** 创作者浏览任务列表
**Then** 列表必须采用分页机制并限制单页数量
**And** 在历史任务规模增长时仍应保持首屏与分页切换的性能基线

**Given** 创作者使用键盘而非鼠标操作工作台
**When** 创作者导航任务导入、列表、详情和下载等关键区域
**Then** 核心流程必须可通过键盘完成
**And** 焦点顺序、焦点可见性和关键操作的语义标签必须清楚可辨

**Given** 页面展示任务状态、错误提示、结果状态或行动按钮
**When** 创作者在桌面端查看这些界面元素
**Then** 关键内容必须满足足够的文本对比度和非纯颜色表达
**And** 屏幕阅读器用户能够获得结构化且可理解的状态信息

**Given** 工作台采用桌面优先布局
**When** 创作者在主要任务路径中使用系统
**Then** 视觉和交互必须遵循已确认的设计基线，包括近中性色、强排版层级、稳定间距节奏和内联关键反馈
**And** 不得退化为重后台式密集配置界面或依赖 toast 承担核心解释职责

**Given** 系统准备进入该 epic 的交付门槛
**When** 团队验证工作台质量基线
**Then** 必须能够通过自动化或人工方式验证关键性能与无障碍要求
**And** 这些验证结果应足以支撑后续 epic 在同一工作台基线之上继续迭代
